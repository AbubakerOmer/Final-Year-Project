...
function myjsapp(peerClient) {
        ...
        $.ajax(
            {
                ...
                success: function (responseData) {
                    if (responseData['result']) {
                        ...
                        {
                            allusers.push(responseData['result'][us])
                        }
                        // alert(allusers)
                        return allusers
    ...
    function EventListeners() {
        ...
        $(document).on('click', '.peeruser', function() {
            console.log("Printing Value", $(this).text())
            var id = $(this).text()
            $('#inputPeerUserId').val(id)
            console.log("Printing 2nd ",$('#inputPeerUserId').val(id))
            connectToPeer()
        });
    ...
    return {
        ...
        createChatWindow: function(id) {
            var toPeerId = id;
            var panel = $('<div class="panel panel-primary chat-div" hidden><div class="panel-heading" hidden></div>' +
            ...
            console.log($('#addAV').html(callButton))

            $('#addAV').append(callButton)



            // var addAV=document.getElementById('addAV')
            // addAV.append(callButton)
        ...
        showIncomingCall : function (peerId, options) {
            ...
            // console.log("AA")
            // alert(typeof allusers)
            // alert(length(allusers))
            // if(allusers.includes(peerId)){
            //     alert('inside if')
...
var myapp,myapp2 ,peerapp;

$(document).ready(function () {
    myapp = myjsapp(peerapp)
    // myapp2 = adoCll(peerapp)
    // myapp = adoCll(peerapp);
    // adoApp=adoCll(peerapp)
